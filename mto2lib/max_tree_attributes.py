...
def compute_attributes(tree_structure, altitudes, run):

    x, y = uts.centroid(tree_structure, run.image.shape[:2])
    ...
    mean, variance = hg.attribute_gaussian_region_weights_model(tree_structure, run.image)
